(() => {
  ...
  async function uploadTransfer(){
    ...
    try{
      ...
      const data=created.data||created; state.transferId=data.transferId; const uploads=data.uploads||[];
      if(!state.transferId||uploads.length!==state.files.length)throw new Error('Upload service returned an unexpected transfer payload.');
      ...
      setProgress(100,'Finalizing transfer…'); await api(`/batch/${encodeURIComponent(state.transferId)}/complete`,{method:'POST',body:'{}'});
